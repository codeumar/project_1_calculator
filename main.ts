...
let intagIn = document.getElementById("n1") as HTMLInputElement;
let intagout = document.getElementById("n2") as HTMLInputElement;
let start;
let end;
type keyvalueobject={
  [key:string] : number
...
let appendx = (val: any) => {
  if (intagIn != null && intagIn.disabled === false) {
    ...
    intagIn.value += val;
    const newVal =
      curr.substring(0, Number(start)) + val + curr.substring(Number(end));
...
Array.from(button).forEach((btn) =>
  btn.addEventListener("click", (e) => {
    ...
    if(ele.innerText=="Add Constant"){
      ...
      if(checkExistance(name as string)){
...
function checkExistance(name:string){
...
function addNewButton(name:any) {
